...
Personal portfolio site. Operations research, stochastic optimization, and the scientific software around them.
...
- **`Experience`** ([src/components/Experience.tsx](src/components/Experience.tsx)) — logo-tile selector + detail panel, split into a primary row (GAMS, Kaizen, Stryker, HES, Femas) and a smaller "Earlier" row. The GAMS entry's blurb is JSX with embedded external links to the GAMSPy SDDP docs, the GAMSPy examples repo, and GAMS Connect — clickable proof embedded in the experience copy.
- **`ProjectMotif`** ([src/components/ProjectMotif.tsx](src/components/ProjectMotif.tsx)) — inline line-art SVGs (flow-shop schedule, simulation–optimization loop, pricing pipeline) standing in for projects with no publishable screenshot. Themed from CSS variables, so all eight palettes work.
...
src/
  ...
  components/
    ...
    Research.tsx          04 — publications, research software, thesis, teaching
    Skills.tsx            05 — uses SkillStrip
    SkillStrip.tsx        Reusable accessible scroll strip
    Projects.tsx          06 — alternating image/text rows
    ProjectMotif.tsx      Drawn stand-ins for image-less projects
    Detour.tsx            07 — the daily puzzle (client component)
    Contact.tsx           08 — the contact form
...
public/
  ...
  Ahmed_Alqershi_Academic_CV.pdf   CV download
```
